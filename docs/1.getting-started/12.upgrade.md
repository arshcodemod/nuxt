...
To help with the upgrade, we’ve worked with the team at [codemod.com](codemod.com) to publish [codemods](https://codemod.com/registry/nuxt-4-migration-recipe) that will automatically update your project to Nuxt 4.

All codemods are available in the [nuxt-codemod](https://codemod.com/registry/nuxt-4-migration-recipe) repo and the Codemod team have joined in helping maintain the codemods. To run these codemods, we recommend using the codemod command.

Run all Nuxt 4 codemods:
...
**Note**:
Codemod default-data-error-value :
```
...
**Note**:
Codemod deprecated-dedupe-value:
```
...
**Note**:
Codemod shallow-data-reactivity:
```
